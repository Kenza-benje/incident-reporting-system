...
public class IncidentResponse {
    ...
    /**
     * Converts a stored file path to a web-accessible URL.
     * Files are saved to uploads/incidents/filename.ext
     * and served via WebConfig at /uploads/incidents/filename.ext
     */
    private static String toWebUrl(String filePath) {
        if (filePath == null) return null;
        // Normalize path separators and extract just the filename portion after uploads/incidents
        String normalized = filePath.replace("\\", "/");
        int idx = normalized.indexOf("uploads/incidents/");
        if (idx >= 0) {
            return "/" + normalized.substring(idx);
        }
        // Fallback: just prepend /uploads/incidents/
    ...
    public static IncidentResponse from(Incident incident) {
        ...
        if (incident.getPhotos() != null && !incident.getPhotos().isEmpty()) {
            List<String> urls = incident.getPhotos().stream()
